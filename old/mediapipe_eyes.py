    ...
    for face_landmarks in results.multi_face_landmarks:
        ...
        if (inside_x - outside_x) != 0:
            ...
            if abs(lx_score - last_x) < .3:
                cv2.putText(img, "x: {:.02}".format(lx_score), (lcx, lcy - 30), cv2.FONT_HERSHEY_SIMPLEX, .25,
                            (255, 255, 255), 1)
            last_x = lx_score
        ...
        if (bottom - top) != 0:
            ...
            if abs(ly_score - last_y) < .3:
                cv2.putText(img, "y: {:.02}".format(ly_score), (lcx, lcy - 20), cv2.FONT_HERSHEY_SIMPLEX, .25,
                            (255, 255, 255), 1)
            last_y = ly_score
